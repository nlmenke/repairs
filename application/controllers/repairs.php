...
class Repairs extends Security_Controller {
    ...
    function edit($ticket_id = -1) {
        if(!$this->ion_auth->logged_in()) {
            ...
            foreach($this->ion_auth->users(array(
                '1',
                '2'
            ))->result() as $employee) {
        ...
        if(!$this->ion_auth->logged_in()) {
            ...
            if($ticket_id == -1) {
                $repair_data = array(
                    ...
                    'date_tested'       => $this->input->post('date_tested') ? date('Y-m-d', strtotime($this->input->post('date_tested'))) : null,
                    'date_called'       => $this->input->post('date_called') ? date('Y-m-d', strtotime($this->input->post('date_called'))) : null,
                    'date_they_called'  => $this->input->post('date_they_called') ? date('Y-m-d', strtotime($this->input->post('date_they_called'))) : null,
