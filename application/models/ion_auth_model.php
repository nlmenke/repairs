...
| Changelog:    03/22/2013 - Additional entropy added
...
class Ion_auth_model extends CI_Model {
    ...
    public function activate($id, $code = false) {
        ...
        if($code !== false) {
            $query = $this->db->select($this->identity_column)->where('activation_code', $code)->limit(1)->get($this->tables['users']);
    ...
    public function deactivate($id = null) {
        ...
        $return = $this->db->affected_rows() == 1;
        if($return) $this->set_message('deactivate_successful'); else
            $this->set_error('deactivate_unsuccessful');
    ...
    public function reset_password($identity, $new) {
        ...
        $query = $this->db->select('id, password, salt')->where($this->identity_column, $identity)->limit(1)->get($this->tables['users']);
    ...
    public function forgotten_password($identity) {
        ...
        if($return) $this->trigger_events(array(
            'post_forgotten_password',
    ...
    public function login($identity, $password, $remember = false) {
        ...
        $query = $this->db->select($this->identity_column.', username, email, id, password, active, last_login')->where($this->identity_column, $this->db->escape_str($identity))->limit(1)->get($this->tables['users']);

        if($query->num_rows() == 0) {
            $query = $this->db->select($this->identity_column.', username, email, id, password, active, last_login')->where('username', $this->db->escape_str($identity))->limit(1)->get($this->tables['users']);
        }
    ...
    public function add_to_group($group_id, $user_id = false) {
        ...
        ))->get($this->tables['users_groups'])->num_rows()
        ) return false;

        if($return = $this->db->insert($this->tables['users_groups'], array(
            $this->join['groups'] => (int)$group_id,
            $this->join['users']  => (int)$user_id
        ))
        ) {
    ...
    public function login_remembered_user() {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column.', id, username, email, last_login')->where($this->identity_column, get_cookie('identity'))->where('remember_code', get_cookie('remember_code'))->limit(1)->get($this->tables['users']);
...
/* Location: ./application/modules/auth/models/ion_auth_model.php */
