...
/*
| Name:			IonAuth MongoDB
| Category:		Models
| Author:		Sepehr Lajevardi
| 				me@sepehr.ws
| Location:		https://github.com/sepehr/ci-mongodb-ionauth-module
| Created:		10/26/2012
| Description:	A rewrite of IonAuth model to use MongoDB as database
| 				backend. It requires both CodeIgniter MongoDB Active
| 				Record and CodeIgniter MongoDB Session libraries
| 				installed. This model class will be loaded in case it's
| 				set to use MongoDB as database backend instead of the
| 				original model class. See controller and library files for
| 				more info on its internal usage.
*/
...
class Ion_auth_mongodb_model extends CI_Model {
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE) {
		...
		$document = $this->mongo_db->select(array('password', 'salt'))
	...
	public function activate($id, $code = FALSE) {
		...
		if($code !== FALSE) {
			...
			$docs = $this->mongo_db->select($this->identity_column)
			...
			$this->trigger_events('extra_where');
			$updated = $this->mongo_db->where($this->identity_column, $identity)
									  ->set(array('activation_code' => NULL, 'active' => 1))
									  ->update($this->collections['users']);
		} else { // Activation code is not set
			$this->trigger_events('extra_where');
	...
	public function clear_forgotten_password_code($code) {
		...
		$count = count($this->mongo_db->where('forgotten_password_code', $code)
									  ->get($this->collections['users']));
	...
	protected function identity_check($identity = '') {
		...
		return count($this->mongo_db->where($this->identity_column, $identity)
									->get($this->collections['users'])) > 0;
	}
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
		...
		if($this->identity_column == 'email' && $this->email_check($email)) {
			...
			return FALSE;
		} elseif($this->identity_column == 'username' && $this->username_check($username)) { // Check if username already exists
	...
	public function login($identity, $password, $remember = FALSE) {
		...
		$document = $this->mongo_db->select(array($this->identity_column, '_id', 'username', 'email', 'password', 'active', 'last_login'))
	...
	public function users($groups = NULL) {
		...
		} elseif(isset($this->_ion_limit)) { // Limit only?
			$this->mongo_db->limit($this->_ion_limit);
			$this->_ion_limit = NULL;
		}
	...
	public function get_users_groups($id = FALSE) {
		...
		$user = $this->mongo_db->select(array('groups'))
							   ->where('_id', new MongoId($id))
							   ->limit(1)
							   ->get($this->collections['users']);
	...
	public function remove_from_group($group_name = FALSE, $user_id = FALSE) {
		...
		if(empty($group_id)) {
			return $this->mongo_db->where('_id', new MongoId($user_id))
								  ->set('groups', array())
								  ->update($this->collections['users']);
		} else { // Only remove the specified group name from the user document
			return $this->mongo_db->where('_id', new MongoId($user_id))
	...
	public function update($id, array $data) {
		...
		$updated = $this->mongo_db->where('_id', new MongoId($user->id))
								  ->set($data)
								  ->update($this->collections['users']);
	...
	public function delete_user($id) {
		...
		$deleted = $this->mongo_db->where('_id', new MongoId($id))
	...
	public function login_remembered_user() {
		...
		$this->trigger_events('extra_where');
		$document = $this->mongo_db->select(array('_id', $this->identity_column))
