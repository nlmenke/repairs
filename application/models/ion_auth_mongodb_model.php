...
/*
| Name:         IonAuth MongoDB Model
| Category:     Models
| Author:       Sepehr Lajevardi
|               me@sepehr.ws
| Location:     https://github.com/sepehr/ci-mongodb-ionauth-module
| Created:
| Copyright:    (c) 2012 Sepehr Lajevardi
| License:      http://codeigniter.com/user_guide/license.html
| Version:      1.0
| Description:  A rewrite of IonAuth model class to use MongoDB as
|               database backend.
...
class Ion_auth_mongodb_model extends CI_Model {
    ...
    public function hash_password_db($id, $password, $use_sha1_override = false) {
        ...
        $document         = $this->mongo_db->select(array(
                'password',
                'salt'
            ))->where('_id', new MongoId($id))->limit(1)->get($this->collections['users']);
    ...
    public function activate($id, $code = false) {
        ...
        if($code !== false) {
            ...
            $docs   = $this->mongo_db->select($this->identity_column)->where('activation_code', $code)->limit(1)->get($this->collections['users']);
            ...
            $this->trigger_events('extra_where');
            $updated = $this->mongo_db->where($this->identity_column, $identity)->set(array(
            ...
            $this->trigger_events('extra_where');
            $updated = $this->mongo_db->where('_id', new MongoId($id))->set(array(
                    'activation_code' => null,
                    'active'          => 1
                ))->update($this->collections['users']);
        }
    ...
    public function clear_forgotten_password_code($code) {
        ...
        $count = count($this->mongo_db->where('forgotten_password_code', $code)->get($this->collections['users']));

        if($count > 0) {
            $this->mongo_db->where('forgotten_password_code', $code)->set(array(
    ...
    public function reset_password($identity, $new) {
        ...
        $docs = $this->mongo_db->select(array(
                ...
                'password',
                'salt'
            ))->where($this->identity_column, $identity)->limit(1)->get($this->collections['users']);
    ...
    public function username_check($username = '') {
        ...
        return count($this->mongo_db->where('username', $username)->get($this->collections['users'])) > 0;
    }
    ...
    public function email_check($email = '') {
        ...
        return count($this->mongo_db->where('email', $email)->get($this->collections['users'])) > 0;
    }
    ...
    protected function identity_check($identity = '') {
        ...
        return count($this->mongo_db->where($this->identity_column, $identity)->get($this->collections['users'])) > 0;
    }
    ...
    public function login($identity, $password, $remember = false) {
        ...
        $document = $this->mongo_db->select(array(
            ...
            // stomach, goveg! :))
            ->where($this->identity_column, (string)$identity)->limit(1)->get($this->collections['users']);
    ...
    public function clear_login_attempts($identity, $expire_period = 86400) {
        ...
        if($this->config->item('track_login_attempts', 'ion_auth')) {
            return $this->mongo_db->where(array(
                ...
                ->or_where(array('time <' => time() - $expire_period))->delete($this->collections['login_attempts']);
        }
    ...
    public function users($groups = null) {
        ...
        if(isset($this->_ion_limit) && isset($this->_ion_offset)) {
            $this->mongo_db->limit($this->_ion_limit)->offset($this->_ion_offset);
            // Flush...
    ...
    public function get_users_groups($id = false) {
        ...
        $user = $this->mongo_db->select(array('groups'))->where('_id', new MongoId($id))->limit(1)->get($this->collections['users']);
    ...
    public function add_to_group($group_id, $user_id = false) {
        ...
        return $this->mongo_db->where('_id', new MongoId($user_id))->push('groups', $group_id)->update($this->collections['users']);
    }
    ...
    public function remove_from_group($group_name = false, $user_id = false) {
        ...
        if(empty($group_id)) {
            return $this->mongo_db->where('_id', new MongoId($user_id))->set('groups', array())->update($this->collections['users']);
        } // Only remove the specified group name from the user document
        else {
            return $this->mongo_db->where('_id', new MongoId($user_id))->pull('groups', array($group_id))->update($this->collections['users']);
        }
    }
    ...
    public function update($id, array $data) {
        ...
        if($this->identity_column !== 'username' && array_key_exists('username', $data) && $this->username_check($data['username']) && $user->username !== $data['username']) {
            $this->set_error('account_creation_duplicate_username');
        ...
        $updated = $this->mongo_db->where('_id', new MongoId($user->id))->set($data)->update($this->collections['users']);
    ...
    public function delete_user($id) {
        ...
        $deleted = $this->mongo_db->where('_id', new MongoId($id))->delete($this->collections['users']);
    ...
    public function update_last_login($id) {
        ...
        return $this->mongo_db->where('_id', new MongoId($id))->set('last_login', time())->update($this->collections['users']);
    }
    ...
    public function remember_user($id) {
        ...
        $updated = $this->mongo_db->where('_id', new MongoId($id))->set('remember_code', $salt)->update($this->collections['users']);
    ...
    public function login_remembered_user() {
        ...
        $document = $this->mongo_db->select(array(
                '_id',
                $this->identity_column
            ))->where($this->identity_column, get_cookie('identity'))->where('remember_code', get_cookie('remember_code'))->limit(1)->get($this->collections['users']);
    ...
    public function update_group($group_id = false, $group_name = false, $additional_data = array()) {
        ...
        $updated = $this->mongo_db->where('_id', new MongoId($group_id))->set($data)->update($this->collections['groups']);
    ...
    public function delete_group($group_id = false) {
        ...
        // delete this group
        $deleted = $this->mongo_db->where('_id', new MongoId($group_id))->delete($this->collections['groups']);
    ...
    protected function _filter_data($collection, $data) {
        ...
        $columns = $collection == 'users' ? // Users collection static schema array
            array(
                '_id',
                'ip_address',
                'username',
